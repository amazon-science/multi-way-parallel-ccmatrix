...
for inp_lp in Counter(dataset["lp"]).keys():
    ...
    with open(f"{res_dir}/ppl_mt_{inp_lp}.pkl", "rb") as fr:
        ppl_mt = pickle.load(fr)

    with open(f"{res_dir}/ppl_ref_{inp_lp}.pkl", "rb") as fr:
        ppl_ref = pickle.load(fr)

    print(f"loaded ppl for mt and src")
    ...
    y_vals_mt = [statistics.median(ppl_per_len_bin_mt[bin_]) for bin_ in x_vals]
    y_vals_ref = [statistics.median(ppl_per_len_bin_ref[bin_]) for bin_ in x_vals]
    ...
    plt.ylabel("Median perplexity with GPT-2")
    plt.title(f"Median perplexity vs Sentence Lengths for {inp_lp}")
    plt.legend()
    plt.savefig(f"./{res_dir}/{inp_lp}_median_ppl.png")
